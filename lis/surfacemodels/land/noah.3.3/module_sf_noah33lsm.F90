...
MODULE module_sf_noah33lsm

  USE module_model_constants_33
...
!  public :: dqsdt
  ...
  public :: snfrac
  PUBLIC :: rosr12
  PUBLIC :: RD, SIGMA, CPH2O, CPICE, LSUBF, EMISSI_S
#if 0
...
!
      ...
      SUBROUTINE SFLX (FFROZP,ICE,ISURBAN,DT,ZLVL,NSOIL,SLDPTH,         &    !C
                       ...
                       TH2,Q2SAT,DQSDT2,                                &    !I
! SLOPE is passed to SFLX instead of SLOPETYP - D. Mocko
                       VEGTYP,SOILTYP,SLOPE,SHDFAC,SHDMIN,SHDMAX,       &    !I
...
! ----------------------------------------------------------------------
                       EVP,ETA,SHEAT, ETA_KINEMATIC,FDOWN,                  &    !O
                       ...
                       hs, rgl, snup, cmcmax, rsmax, topt, bexp, dksat, & 
                       dwsat, f1, quartz, psisat, czil, sbeta, fxexp,   & 
...
! 1. CONFIGURATION INFORMATION (C):
! ----------------------------------------------------------------------
      INTEGER, INTENT(IN) ::  ICE,NSOIL,SOILTYP,VEGTYP
...
! ----------------------------------------------------------------------
      ...
      REAL,INTENT(OUT)   :: EVP, ETA_KINEMATIC,BETA,DEW,DRIP,EC,EDIR,ESNOW,ETA,  &
...
! ----------------------------------------------------------------------
         ...
         ELSE
            SNDENS = SNEQV / SNOWH
            IF(SNDENS > 1.0) THEN
             print*,'Physical snow depth is less than snow water equiv.' 
             stop
            ENDIF
...
! Initialize DTOT and FRCSNO even if there is no snow - D. Mocko
            DTOT = 0.0
            FRCSNO = 0.0
         ELSE
            DTOT = SNOWH + DSOIL
...
! Initialize variables in case they are printed out - D. Mocko
         T24=0.0
         RCH=0.0
         EPSCA=0.0
         RR=0.0
...
! ----------------------------------------------------------------------
! Fixed calculation of total evapotranspiration in (KG M-2 S-1) before 
! accounting for latent heat values for different phases. - D. Mocko
      IF (ETP .GT. 0.) THEN
        EVP = EDIR + EC + ETT + ESNOW
      ELSE
        EVP = ETP
      ENDIF
      EDIR = EDIR * LVH2O
...
!
               IF (SOILTYP .gt. SLCATS) THEN
                        print*, 'Warning: too many input soil types' 
                        stop
               END IF
               IF (VEGTYP .gt. LUCATS) THEN
                     print*, 'Warning: too many input landuse types' 
                     stop
               END IF
               IF (SLOPETYP .gt. SLPCATS) THEN
                     print*, 'Warning: too many input slope types' 
                     stop
               END IF
...
! ----------------------------------------------------------------------
      ...
      ALBEDOMAX = ALBEDOMAXTBL (VEGTYP)
               ...
               IF (NROOT .gt. NSOIL) THEN
                  WRITE (*,*) 'Error: too many root layers ',  &
                                                 NSOIL,NROOT
                  stop
! ----------------------------------------------------------------------
...
! ----------------------------------------------------------------------
      ...
      SUBROUTINE SFCDIF_off (ZLM,ZLM_WIND,Z0,THZ0,THLM,SFCSPD,CZIL,AKMS,AKHS,&
           VEGTYP, ISURBAN, IZ0TLND, USTAR)
#else
      SUBROUTINE SFCDIF_off (ZLM,Z0,THZ0,THLM,SFCSPD,CZIL,AKMS,AKHS,USTAR)
#endif
...
real function month_d(a12,nowy,nowm,nowd) result (nowval)
  !
  ...
  ! 
  ! Return a value valid for the day given in <nowy>,<nowm>,<nowd>,
  ! as an interpolation from the 12 monthly <a12> values.
  !
  implicit none
...
END MODULE module_sf_noah33lsm
