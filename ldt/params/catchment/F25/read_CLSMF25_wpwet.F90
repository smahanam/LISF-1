...
!BOP
!
! !ROUTINE: read_CLSMF25_wpwet
!  \label{read_CLSMF25_wpwet}

! !REVISION HISTORY:
!  25 Nov 2012: K. Arsenault; Initial Specification
!
! !INTERFACE:
subroutine read_CLSMF25_wpwet(n,array,maskarray) 
...
!   index of the nest
!  \item[array]
!   output field for wilting point wetness
!  \item[maskarray]
...
!- Determine global/complete parameter domain number of points:
   ...
   write(LDT_logunit,*) "[INFO] Reading wilting point wetness from: ",trim(CLSMF25_struc(n)%soilparamfile)
...
!- For now - complete domains:
   do r = 1, glpnr
      do c = 1, glpnc
         ...
         !- Wilting point wetness parameter:
!            array(c,r) = tmpreal(5)   
            tmparray(c,r) = tmpreal(5)   
...
end subroutine read_CLSMF25_wpwet
