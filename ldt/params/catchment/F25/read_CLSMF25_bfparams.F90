...
!BOP
!
! !ROUTINE: read_CLSMF25_bfparams
!  \label{read_CLSMF25_bfparams}

! !REVISION HISTORY:
!  25 Nov 2012: K. Arsenault; Initial Specification
!
! !INTERFACE:
subroutine read_CLSMF25_bfparams(n,array1,array2,array3,maskarray) 
...
!
! !DESCRIPTION:
!  This subroutine retrieves the Catchment LSM topographic baseflow
!  (bf) parameters from a text-based formatted file and places it on
...
!   index of the nest
!  \item[array1]
!   output field for bf1 parameter
!  \item[array2]
!   output field for bf2 parameter
!  \item[array3]
!   output field for bf3 parameter
!  \item[maskarray]
...
!- Determine global/complete parameter domain number of points:
   ...
   inquire(file=trim(CLSMF25_struc(n)%topo_bf_file), exist=file_exists)
   if(.not.file_exists) then 
     write(LDT_logunit,*) "Catchment F2.5 topographic baseflow &
               (bf) parameter file ",trim(CLSMF25_struc(n)%topo_bf_file)," not found."
     write(LDT_logunit,*) "Program stopping ..."
     call LDT_endrun
   endif
  
   ftn = LDT_getNextUnitNumber()
   write(LDT_logunit,*) "[INFO] Reading topographic baseflow (bf) data &
             from: ",trim(CLSMF25_struc(n)%topo_bf_file)
   open(ftn, file=CLSMF25_struc(n)%topo_bf_file, form="formatted")

!- Loop over the CLSM mask values and read in each line from the baseflow file:
...
end subroutine read_CLSMF25_bfparams
