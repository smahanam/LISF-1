...
!BOP
!
! !ROUTINE: read_CLSMF25_araparams
!  \label{read_CLSMF25_araparams}

! !REVISION HISTORY:
!  25 Nov 2012: K. Arsenault; Initial Specification
!
! !INTERFACE:
subroutine read_CLSMF25_araparams(n,array1,array2,array3,array4,maskarray) 
...
!
! !DESCRIPTION:
!  This subroutine retrieves the Catchment LSM topographic shape 
!  (ara) parameters from a text-based formatted file and places it on
...
!   index of the nest
!  \item[array1]
!   output field for ara1 parameter
!  \item[array2]
!   output field for ara2 parameter
!  \item[array3]
!   output field for ara3 parameter
!  \item[array4]
!   output field for ara4 parameter
!  \item[maskarray]
...
!- Determine global/complete parameter domain number of points:
   ...
   if(.not.file_exists) then 
     write(LDT_logunit,*) "Catchment F2.5 topographic shape &
               (ara) parameter file ",trim(CLSMF25_struc(n)%topo_ar_file)," not found."
     write(LDT_logunit,*) "Program stopping ..."
     call LDT_endrun
   endif
  
   ftn = LDT_getNextUnitNumber()
   write(LDT_logunit,*) "[INFO] Reading topographic shape (ara) data &
...
!- For now - complete domains:
   do r = 1, glpnr
      do c = 1, glpnc
         ...
         !- Shape parameter:
!            array1(c,r) = tmpreal(5)   
!            array2(c,r) = tmpreal(6)   
!            array3(c,r) = tmpreal(7)   
!            array4(c,r) = tmpreal(8)   
            tmparray1(c,r) = tmpreal(5)   
            tmparray2(c,r) = tmpreal(6)   
            tmparray3(c,r) = tmpreal(7)   
            tmparray4(c,r) = tmpreal(8)   
...
end subroutine read_CLSMF25_araparams
