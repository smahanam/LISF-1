...
!BOP
!
! !ROUTINE: read_CLSMF25_bedrockdepth
!  \label{read_CLSMF25_bedrockdepth}

! !REVISION HISTORY:
!  25 Nov 2012: K. Arsenault; Initial Specification
!
! !INTERFACE:
subroutine read_CLSMF25_bedrockdepth(n,array,maskarray) 
...
!   index of the nest
!  \item[array]
!   output field for depth to bedrock (in meters)
!  \item[maskarray]
...
!- Determine global/complete parameter domain number of points:
   ...
   inquire(file=trim(CLSMF25_struc(n)%soilparamfile), exist=file_exists)
   if(.not.file_exists) then 
     write(LDT_logunit,*) "Catchment F2.5 soil parameter file ",&
                     trim(CLSMF25_struc(n)%soilparamfile)," not found."
     write(LDT_logunit,*) "Program stopping ..."
     call LDT_endrun
   endif
  
   ftn = LDT_getNextUnitNumber()
   write(LDT_logunit,*) "[INFO] Reading soil parameters from: ",trim(CLSMF25_struc(n)%soilparamfile)
   open(ftn, file=CLSMF25_struc(n)%soilparamfile, form="formatted")
...
!- For now - complete domains:
   do r = 1, glpnr
      do c = 1, glpnc
         ...
         !- Bed rock depth parameter:
            tmparray(c,r) = tmpreal(6) + CLSMF25_struc(n)%addbdrckcrd*1000. ! add to for the GRACE work
...
end subroutine read_CLSMF25_bedrockdepth
