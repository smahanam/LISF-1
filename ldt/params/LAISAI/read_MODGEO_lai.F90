...
!BOP
!
! !ROUTINE: read_MODGEO_lai
! \label{read_MODGEO_lai}
!
! !REVISION HISTORY:
!  27 Oct 2019: KR Arsenault and Sarith Mahanama ; first version
!
! !INTERFACE:
subroutine read_MODGEO_lai(n, array, maskarray)
...
! !ARGUMENTS: 
  integer, intent(in)    :: n
  real,    intent(inout) :: array(LDT_rc%lnc(n),LDT_rc%lnr(n),46)
...
!  specified 8-day period and returns the values for Catchment J3.2. 
!  
!  The arguments are:
!  \begin{description}
!  \item[n]
!   time index (octad, month or quarter)
!  \item[array]
...
!- Determine global/complete parameter domain number of points:
   ...
   allocate( tmparray(glpnc,glpnr,46) )
      ...
      do r = 1, glpnr
         do c = 1, glpnc
            if( LDT_rc%global_mask(c,r) > 0. ) then
              ...
              k = k + 1
              if( tmpreal(k) < 0 ) then
                 tmparray(c,r,octad) = LDT_rc%udef
              else
                 tmparray(c,r,octad) = tmpreal(k)
              endif
           endif
           if( tmparray(c,r,octad) < 0.01 ) tmparray(c,r,octad) = 0.01
   ...
   !- Subset domain:
      do r = 1, LDT_rc%lnr(n)
         do c = 1, LDT_rc%lnc(n)
            ...
            array(c,r,octad) = tmparray(gc,gr,octad)
         end do
      end do

   end do  ! end octad loop
   deallocate( tmparray )

   call LDT_releaseUnitNumber(ftn)

end subroutine read_MODGEO_lai
