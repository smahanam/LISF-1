...
! _____________________________________________________________
   ...
   do n=1,LDT_rc%nnest
      ...
      if(LDT_gfrac_struc(n)%gfrac%selectOpt.gt.0) then 
         ...
         if(trim (LDT_gfrac_struc(n)%gfrac%source) .eq. "GSWPHG") then
   ...
   !- Greenness fraction:
   if( gfrac_select ) then 
      if(trim (LDT_gfrac_struc(1)%gfrac%source) .ne. "GSWPHG") then
      ...
      do n=1,LDT_rc%nnest
         ...
         if(( trim(gfracInterval(n)) .ne. "monthly" ).and.(trim (LDT_gfrac_struc(n)%gfrac%source) .ne. "GSWPHG")) then 
   ...
   do n = 1, LDT_rc%nnest
      ...
      !- Monthly (interval) files:
         ...
         !- Read single-file monthly clim greenness fraction: 
            ...
            elseif ( trim(LDT_gfrac_struc(n)%gfrac%source) == "GSWPHG" ) then
               write(LDT_logunit,*) "Reading single-file, monthly climatologies for: GSWPHG"
               call readgfrac( trim(LDT_gfrac_struc(n)%gfrac%source)//char(0),&
                    n, LDT_gfrac_struc(n)%gfrac%value, &
                    LDT_LSMparam_struc(n)%landmask%value )
               write(LDT_logunit,*) "Done reading file - GSWPHG"
